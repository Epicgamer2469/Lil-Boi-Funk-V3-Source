...
class PlayState extends MusicBeatState
{
	...
	var vignetteLerp:Bool = false;
	var vignetteDef:Float = .5;
	...
	var shitSound:FlxSound = new FlxSound().loadEmbedded(Paths.sound('shitting', 'shared'));

	var vignette:BGSprite;
	var vignette2:BGSprite;
	var tarrVignette:BGSprite;

	var rockyBop:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'rocky':
				var back:BGSprite = new BGSprite('lbf/rocky/back', -600, -300);

				grottoBopper = new BGSprite("lbf/rocky/boppers", 250, 325, 1, 1, ['bopping']);
				grottoBopper.scale.set(.875, .875);

				add(back);

				add(grottoBopper);
				grottoBopper.visible = false;
				rockyBop = new BGSprite("lbf/rocky/boppers2", -200, 250, 1, 1, ['bopping2']);
				rockyBop.visible = false;
				add(rockyBop);
			case 'grotto':
				var back:BGSprite = new BGSprite("lbf/grotto/grotto", -600, -300., .9, .9);

				grottoBopper = new BGSprite("lbf/grotto/phosphorbop", -500, 92, .9, .9, ['bop']);

				add(back);
				add(grottoBopper);
			case 'shit':
				var bg:BGSprite = new BGSprite("shit/bg");
				bg.scale.set(1.75, 1.75);
				add(bg);

				var fore:BGSprite = new BGSprite("shit/for");
				fore.scale.set(1.75, 1.75);
				add(fore);
		}
		...
		if(dad.curCharacter.startsWith('hotchick')) {
			dad.setPosition(GF_X, GF_Y);
			gf.visible = false;
		}
...
/* 		doof.nextDialogueThing = startNextDialogue;
		...
		switch(curSong.toLowerCase())
		{
			case 'tutorial':
				dad.y += 200;
			case 'stack-attack':
				grottoBopper.visible = true;
			case 'rock hard':
				rockyBop.visible = true;
			case 'starrvation':
				dad.visible = false;
		}
		...
		vignette = new BGSprite("lbf/grotto/normal", 0, 0, 0, 0);

		if(curSong.toLowerCase() == 'cave rave' || curSong.toLowerCase() == 'spelunkers')
			{
				vignette.alpha = .5;
			}
		else
		...
		if(curSong.toLowerCase() == 'starrvation')
			{
				tarrVignette = new BGSprite("lbf/grotto/slimy", 0, 0, 0, 0);
				add(tarrVignette);
				tarrVignette.cameras = [camOther];
				remove(grottoBopper);
			}
		add(vignette);
		vignette.cameras = [camOther];

		vignette2 = new BGSprite("lbf/grotto/normal", 0, 0, 0, 0);
		vignette2.alpha = 0;
		add(vignette2);
		vignette2.cameras = [camOther];

		if(FlxG.random.bool(1)){
			var peter:FlxSprite;
			peter = new FlxSprite().loadGraphic(Paths.image(FlxG.random.bool(75) ? 'peterTheSequel' : 'peter', 'shared'));
			peter.antialiasing = ClientPrefs.globalAntialiasing;
			peter.scale.set(.15, .15);
			peter.updateHitbox();
			peter.x = FlxG.width - peter.width;
			peter.y = FlxG.height - peter.height;
			add(peter);
			peter.cameras = [camOther];
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
				case 'tutorial' | 'towering-up' | 'stack-attack' | 'rock-hard'| 'cave-rave' | 'spelunkers' | 'starrvation':
					schoolIntro(doof);
	...
	override public function update(elapsed:Float)
	{
		...
		if(vignetteLerp)
			vignette.alpha = FlxMath.lerp(vignetteDef, vignette.alpha, .95);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Change Camera Zoom':
				...
				modchartTweens.set('zoomTween', FlxTween.tween(FlxG.camera, {zoom: newZoom}, Std.parseFloat(value2), {ease: FlxEase.quadOut, onComplete: function(twn:FlxTween){
					modchartTweens.remove('zoomTween');
					defaultCamZoom = newZoom;
				}}));
				//defaultCamZoom = newZoom;
			case 'Camera Flash':
				//color,duration
				var camValues:Array<String> = value2.split(',');
				////////////////////////////////////////////////////////////trace(camValues);
				switch(value1)
					{
						case 'camHUD':
							if(ClientPrefs.flashing)
								camHUD.flash(FlxColor.fromString(camValues[0]), Std.parseFloat(camValues[1]));//Std?? more like gonnehhrea
						case 'gameCam':
							if(ClientPrefs.flashing)
								FlxG.camera.flash(FlxColor.fromString(camValues[0]), Std.parseFloat(camValues[1]));
					}
			case 'Change Vignette Alpha':
				var values:Array<String> = value2.split(',');
				var pooSprite:BGSprite;
				if(value1 == '1')
					pooSprite = vignette;
				else
					pooSprite = vignette2;

				modchartTweens.set('vignetteTween', FlxTween.tween(pooSprite, {alpha: Std.parseFloat(values[0])}, Std.parseFloat(values[1]), {ease: FlxEase.quadOut, onComplete: function(twn:FlxTween){
					modchartTweens.remove('vignetteTween');
				}}));
			case 'Vignette Lerp':
				//addamount, default
				if(value1 == ''){
					...
					vignetteLerp = true;
					vignette.alpha += Std.parseFloat(value1);
					vignetteDef = Std.parseFloat(value2);
				}
		}
	...
	public function moveCamera(isDad:Bool) {
		if(isDad) {
			...
			maxCamera = [camFollow.x - 50, camFollow.y + 50, camFollow.y - 50, camFollow.x + 50];
...
/* 		if (Paths.formatToSongPath(SONG.song) == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1.3) {
	...
	function finishSong():Void
	{
		...
		if(curSong.toLowerCase() == 'rock hard' && isStoryMode)
			{
				var file = Paths.txt('rock-hard/endDialogue');
				dialogue = CoolUtil.coolTextFile(file);
				finishCallback = endDialogue;
			}
		updateTime = false;
	...
	override function beatHit()
	{
		...
		if(curSong == 'starrvation' && curBeat == 16)
			dad.visible = true;
		...
		{
			case 'grotto' | 'rocky':
				if(curSong.toLowerCase() != 'rock hard')
					grottoBopper.dance(true);
				else
					rockyBop.dance(true);
			case 'school':
				if(!ClientPrefs.lowQuality) {
