...
class TitleState extends MusicBeatState
{
	...
	#if debug
	var poopBool:Bool = FlxG.random.bool(5);
	#else
	var poopBool:Bool = FlxG.random.bool(5);
	#end
	...
	var pissTween:FlxTween;
	var cumTween:FlxTween;
	var textGroup:FlxGroup;
	var logoSpr:FlxSprite;
	var poopSprite:FlxSprite;
	...
	var easterEggKeyCombination:Array<FlxKey> = [FlxKey.P, FlxKey.E, FlxKey.E];
	...
	function startIntro()
	{
		...
		poopSprite = new FlxSprite().loadGraphic(Paths.image('startBG'));
		poopSprite.alpha = 0;
		add(poopSprite);

		logoBl = new FlxSprite(-150, -1400);
		if (poopBool)
			logoBl.frames = Paths.getSparrowAtlas('logoBumpinLOL');
		else 
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['Mod By'], -69);
				// credTextShit.visible = true;
				case 3:
					addMoreText('Team Mid');
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['Phsych Engine by'], -60);
				case 7:
					addMoreText('Shadow Mario', -60);
					addMoreText('RiverOaken', -60);
				...
				// credTextShit.screenCenter();
				case 13:
					pissTween = FlxTween.tween(FlxG.camera, {zoom: 1.5}, 2, {ease:FlxEase.quartIn});
					cumTween = FlxTween.tween(poopSprite, {alpha: 1}, 2, {onComplete: function(twn:FlxTween){
						trace('cummy wummy yummy');
					}});
					addMoreText('Lil');
				// credTextShit.visible = true;
				case 14:
					if (FlxG.random.bool(5))
						addMoreText('poopTween');
					else
						addMoreText('Boi');
				// credTextShit.text += '\nNight';
				case 15:
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			var poopTween:FlxTween = FlxTween.tween(logoBl, {y: -50}, 1.5, {ease: FlxEase.quartOut});
